...
def render_page(
    ...
    """Render one page and return words aligned to the rendered image.

    Searchable PDFs do not need a high-resolution image for OCR. Detect their
    embedded text first and render them at only 120 DPI, which makes the
    one-click path much faster. Scanned pages still use the selected OCR DPI.
    """
    with fitz.open(str(pdf_path)) as document:
        ...
        effective_dpi = min(int(dpi), 120) if text_layer_is_usable(probe_words, full_text) else int(dpi)
